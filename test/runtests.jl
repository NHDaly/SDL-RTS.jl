...
@testset "willCollide(collector, paddle, dt)" begin
@testset "dt" begin
b = Collector(WorldPos(-10,-1), Vector2D(0,2))
...
b = Collector(WorldPos(0,0), Vector2D(0,-2))
p = Paddle(WorldPos(0,-100), kZeroVel, 10)
@test !willCollide(b, p, 1)
end

@testset "collision detection" begin
b = Collector(WorldPos(-10,9), Vector2D(0,2))
p = Paddle(WorldPos(-8,10), kZeroVel, 5)
@test willCollide(b, p, 1)
b = Collector(WorldPos(-10,11), Vector2D(0,2))
@test !willCollide(b, p, 1)

b = Collector(WorldPos(0,-195), Vector2D(0,-200))
p = Paddle(WorldPos(0,-200), kZeroVel, 5)
@test willCollide(b, p, 1)
b = Collector(WorldPos(0,-200), Vector2D(0,-200000))
@test willCollide(b, p, 1)
@test willCollide(b, p, 0)
b = Collector(WorldPos(0,0), Vector2D(0,-200))
@test willCollide(b, p, 1)
b = Collector(WorldPos(0,0), Vector2D(0,-200))
@test willCollide(b, p, 1)
end
